...
TRIGGER_DESCRIPTIONS = {
    ...
    "Social_Proof_Bias": "social proof and peer influence",
...
class ExplanationGenerator:
    ...
    def generate(
        ...
        try:
            explanation = self._call_gemini(cognitive_trigger, risk_delta, page_context)
            logger.info(
                f"[ExplanationGenerator] Gemini explanation generated for {user_id} "
                f"trigger={cognitive_trigger} delta={risk_delta}"
            )
        except Exception as e:
            logger.warning(f"[ExplanationGenerator] Gemini failed: {e} — trying Ollama")
            try:
                explanation = self._call_ollama(cognitive_trigger, risk_delta, page_context)
                logger.info(f"[ExplanationGenerator] Ollama explanation generated for {user_id}")
            except Exception as e2:
                logger.warning(f"[ExplanationGenerator] Ollama failed: {e2} — using fallback template")
        ...
        if event_id:
            try:
                ...
                logger.info(
                    f"[ExplanationGenerator] Stored explanation in Neo4j event_id={event_id}"
                )
            except Exception as e:
                logger.warning(f"[ExplanationGenerator] Neo4j store failed: {e}")
    ...
    def publish_to_redis(self, user_id: str, alert_payload: dict):
        try:
            ...
            logger.info(
                f"[ExplanationGenerator] Published alert to Redis channel=alerts:{user_id}"
            )
        except Exception as e:
            logger.warning(f"[ExplanationGenerator] Redis publish failed: {e}")

    def _call_gemini(self, cognitive_trigger: str, risk_delta: int, page_context: str) -> str:
        if not self.client:
            raise Exception("Gemini API key not configured")
        trigger_desc = TRIGGER_DESCRIPTIONS.get(cognitive_trigger, "deception tactics")
...
Be warm, encouraging, and specific to the message content above."""
    ...
    def _call_ollama(self, cognitive_trigger: str, risk_delta: int, page_context: str) -> str:
        trigger_desc = TRIGGER_DESCRIPTIONS.get(cognitive_trigger, "deception tactics")
...
Be warm, encouraging, and specific to the message content above."""
    ...
    def _fallback_explanation(self, cognitive_trigger: str, risk_delta: int) -> str:
